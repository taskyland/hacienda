...
type TypedHeaders = Partial<Record<HTTPHeaderName, unknown>> &
  Partial<{
    host: string
...
function formatEventStreamMessage(message: EventStreamMessage): string {
  let result = ''
  if (message.id) {
...
function formatEventStreamMessages(messages: EventStreamMessage[]): string {
  let result = ''
...
function isHttp2Request(event: H3Event) {
  return (
